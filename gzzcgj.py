...
def freedown(username,passwd):
    ...
    res = req.get('http://www.gzpi.gov.cn/vsgzhr/PHJG/评后结果高级2.html',headers=header)
...
#        print so.find_all('td')[1]
        for title in so.find_all ('p'):
            if len(title.text) == 0 or len(title.text) > 50 :
                    continue
            if ewyh in title.text:
                print '****************************************************'
                print title.text
                #print n
                print '****************************************************'
                
            if snd in title.text:
                print '----------------'
                print title.text
                #print n
                print '----------------'
        for td_url in so.find_all('td'):
            ...
            if len(td_url.text) == 0 or len(td_url.text) > 50 :
                    continue
            if ewyh in td_url.text:              
                print '****************************************************'
                print td_url.text
                #print n
                print '****************************************************'
                
            if snd in td_url.text:
                print '----------------'
                print td_url.text
                #print n
                print '----------------'
            elif suser in td_url.text:
                ...
                if len(td_url.text) > 50 or len(td_url.text) == 0 :
                    ...
                    print td_url.text + ' | ' + so.find_all('td')[n].text + ' | ' +  so.find_all('td')[n+1].text
